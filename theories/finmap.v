...
Lemma pred_of_finsetE (K : keyType) (f : finSet K) k :
...
Variables (K : keyType) (A : finSet K).
...
Definition fset_sub_choiceMixin := PcanChoiceMixin fset_sub_pickleK.
Canonical fset_sub_choiceType :=
  Eval hnf in ChoiceType fset_sub fset_sub_choiceMixin.
...
Lemma fsubsetAA A : A `<=` A. Proof. exact/fsubsetP. Qed.
...
Canonical finmap_of_finfun (K : keyType) V (A : {fset K}) (f : {ffun A -> V}) := FinMap f.
...
Variables (K : keyType) (V : keyType).
Implicit Type (f : {fmap K -> V}).
...
(* apply/fsetP => v'. *)
(* case: ifPn => [/'exists_andP [k' [neq_k'k] /eqP fk'_eq]|]. *)
(*   apply/codomfP/codomfP => [[k'']|[k'']]. *)
(*     by rewrite fnd_rem in_fsetE; have [//|neq_kk'] := altP eqP; exists k''. *)
(*   have [<- fk''|] := eqVneq (f k') v'. *)
(*     by exists (val k'); rewrite fnd_rem1 neq_k'k /= in_fnd getfE. *)
(*   rewrite fk'_eq => fk fk''; exists k''. *)
(*   rewrite fnd_rem1; have [eq_k''k /=|//] := altP eqP. *)
(*   by move: fk'' fk; rewrite /= eq_k''k in_fnd => [] [->]; rewrite eqxx. *)
(* rewrite negb_exists => /forallP /= fP; rewrite !in_fsetE /=. *)
(* apply/codomfP/idP => [[k']|/andP [v'neq /codomfP [k' /fndSomeP [k'f fk']]]]. *)
(*   rewrite fnd_rem1; have [//|neq_kk' /= /fndSomeP [k'f fk']] := altP eqP. *)
(*   have := fP.[k'f]; rewrite /= neq_kk' /= fk' => -> /=. *)
(*   by apply/codomfP; exists k'; rewrite in_fnd fk'. *)
(* exists k'; rewrite fnd_rem1 in_fnd fk'; have [eq_k'k|] //= := altP eqP. *)
(* suff : f.[? k'] != f.[? k] by rewrite eq_k'k eqxx. *)
(* by rewrite !in_fnd fk' (inj_eq Some_inj). *)
(* Qed. *)
