...
Definition rW_nominal_setoid_mixin :=
  @PermSetoidMixin _ rW (@eq rW) rW_act rW_act1
...
Definition rW_nominal_mixin  :=
  @NominalMixin _ rW_choiceType rW_nominal_setoid_mixin _
                rW_act_id.

Canonical rW_nominalType :=
  NominalType atom rW rW_nominal_mixin.
...
Definition W_nominal_setoid_mixin :=
  @PermSetoidMixin _ W (@eq W) W_act W_act1 W_actM W_actproper.
...
Definition W_nominal_mixin :=
  @NominalMixin _ _ W_nominal_setoid_mixin _ W_act_id.

Canonical W_nominalType := NominalType atom W W_nominal_mixin.
...
elim/W_naive_ind : u => [x|c a f IHf|c x a f IHf] π.
  ...
  - rewrite BCons_equivariant.
    pose z := fresh_in (C, π \dot x, π \dot f); freshTacCtxt z.
...
Lemma equiv_res_subproof c (x : atom) a f :
(forall i, equiv (f i)) ->
x # Supp -> @res c x a f.
Proof.
move => Hequiv xFSupp.
