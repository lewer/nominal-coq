...
Lemma big_rel_trans T (b : big_rel_of T) : transitive b.
Proof. by case: b => [? []]. Qed.
...
by [exact: leq_trans| rewrite (leq_maxl, leq_maxr)].
Qed.
...
Definition big_type T of rel T := T.

Ltac pose_big_enough_gen l i :=
  evar (i : big_type l); suff : closed i; first do
    [move=> _; instantiate (1 := bigger_than l _) in (Value of i)].

Ltac pose_big_enough i := pose_big_enough_gen leq i.

Ltac pose_big_modulus_gen l F m :=
  evar (m : F -> big_type l); suff : closed m; first do
    [move=> _; instantiate (1 := (fun e => bigger_than l _)) in (Value of m)].

Ltac pose_big_modulus m F := pose_big_modulus_gen leq F m.
...
Ltac big_enough_trans :=
  match goal with
    | [leq_nm : is_true (?l ?n ?m) |- is_true (?l ?x ?m)] =>
        apply: big_rel_trans leq_nm; big_enough; olddone
