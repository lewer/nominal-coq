...
Definition atomact aso π (a : asort * atom) := if aso == a.1 then (a.1, π a.2)
                                               else a.

Lemma atomact1 aso : forall (a : asort * atom), atomact aso 1 a = a.
Proof.
move => a /=; rewrite /atomact finsfun1. 
by case: a => /= ? ?; rewrite if_same.
Qed.

Lemma atomactM aso : 
  forall π π' a, atomact aso (π * π') a = atomact aso π (atomact aso π' a).
Proof. 
move => π π' a /=; rewrite /atomact finsfunM. 
by have [->| /negPf ->] := boolP (aso == a.1) => //=. 
Qed.
...
Definition atom_nominal_setoid_mixin :=
  @PermSetoidMixin _ asort (asort * atom) (@eq (asort * atom)) atomact atomact1 atomactM atomactproper.

Definition atom_support aso (a : asort * atom) :=
  if aso == a.1 then [fset a.2] else fset0.

Lemma atomact_id aso π a :
     pfixe π (atom_support aso a) -> atomact aso π a = a.
Proof. 
case: a => asoa a /=. rewrite/atom_support/atomact/=. 
case: (aso == asoa) => //= πfixea.  
by rewrite πfixea // in_fset1 eqxx.
Qed.

Definition atom_nominal_mixin :=
  @NominalMixin _ asort (asort * atom) atom_nominal_setoid_mixin _ atomact_id.

Canonical atom_nominal_type := NominalType atom asort (asort * atom) atom_nominal_mixin.
